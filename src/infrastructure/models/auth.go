...
type Auth struct {
	...
	ChangePasswordCount int    `boil:"change_password_count" json:"change_password_count" toml:"change_password_count" yaml:"change_password_count"`
	FailAuthCount       int    `boil:"fail_auth_count" json:"fail_auth_count" toml:"fail_auth_count" yaml:"fail_auth_count"`
	// 0:削除, 1:利用可能, 2:一時停止
...
var AuthColumns = struct {
	...
	Password            string
	Session             string
	ChangePasswordCount string
	FailAuthCount       string
	Status              string
	CreatedAt           string
	UpdatedAt           string
}{
	...
	Password:            "password",
	Session:             "session",
	ChangePasswordCount: "change_password_count",
	FailAuthCount:       "fail_auth_count",
	Status:              "status",
...
var AuthTableColumns = struct {
	...
	ChangePasswordCount: "auth.change_password_count",
	FailAuthCount:       "auth.fail_auth_count",
	Status:              "auth.status",
...
var AuthWhere = struct {
	...
	ChangePasswordCount whereHelperint
	FailAuthCount       whereHelperint
	Status              whereHelperint
	CreatedAt           whereHelpertime_Time
	UpdatedAt           whereHelpertime_Time
}{
	WorkerID:            whereHelperstring{field: "`auth`.`worker_id`"},
	Password:            whereHelperstring{field: "`auth`.`password`"},
	Session:             whereHelperstring{field: "`auth`.`session`"},
	ChangePasswordCount: whereHelperint{field: "`auth`.`change_password_count`"},
	FailAuthCount:       whereHelperint{field: "`auth`.`fail_auth_count`"},
	Status:              whereHelperint{field: "`auth`.`status`"},
	CreatedAt:           whereHelpertime_Time{field: "`auth`.`created_at`"},
	UpdatedAt:           whereHelpertime_Time{field: "`auth`.`updated_at`"},
}
...
type authR struct {
	Worker *Worker `boil:"Worker" json:"Worker" toml:"Worker" yaml:"Worker"`
}
...
var (
	authAllColumns            = []string{"worker_id", "password", "session", "change_password_count", "fail_auth_count", "status", "created_at", "updated_at"}
	authColumnsWithoutDefault = []string{"worker_id", "password", "session", "change_password_count", "fail_auth_count", "status", "created_at", "updated_at"}
	authColumnsWithDefault    = []string{}
	authPrimaryKeyColumns     = []string{"worker_id"}
...
// Worker pointed to by the foreign key.
func (o *Auth) Worker(mods ...qm.QueryMod) workerQuery {
	...
	return Workers(queryMods...)
}

// LoadWorker allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (authL) LoadWorker(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAuth interface{}, mods queries.Applicator) error {
	var slice []*Auth
	var object *Auth
	...
	query := NewQuery(
		qm.From(`workers`),
		qm.WhereIn(`workers.worker_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load Worker")
	}

	var resultSlice []*Worker
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice Worker")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for workers")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for workers")
	}
...
// SetWorker of the auth to the related item.
// Sets o.R.Worker to related.
// Adds o to related.R.Auth.
func (o *Auth) SetWorker(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Worker) error {
	var err error
	if insert {
	...
	values := []interface{}{related.WorkerID, o.WorkerID}
...
func FindAuth(ctx context.Context, exec boil.ContextExecutor, workerID string, selectCols ...string) (*Auth, error) {
	authObj := &Auth{}
	...
	query := fmt.Sprintf(
		"select %s from `auth` where `worker_id`=?", sel,
	)
...
func (o *Auth) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `auth` WHERE `worker_id`=?"
...
func (o *Auth) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindAuth(ctx, exec, o.WorkerID)
	if err != nil {
		return err
	}
...
func AuthExists(ctx context.Context, exec boil.ContextExecutor, workerID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `auth` where `worker_id`=? limit 1)"
...
func (o *Auth) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AuthExists(ctx, exec, o.WorkerID)
}
