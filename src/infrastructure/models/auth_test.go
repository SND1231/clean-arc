...
func testAuthsExists(t *testing.T) {
	...
	e, err := AuthExists(ctx, tx, o.WorkerID)
	if err != nil {
...
func testAuthsFind(t *testing.T) {
	...
	authFound, err := FindAuth(ctx, tx, o.WorkerID)
	if err != nil {
		t.Error(err)
	}
...
func testAuthToOneWorkerUsingWorker(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, workerDBTypes, false, workerColumnsWithDefault...); err != nil {
	...
	local.WorkerID = foreign.WorkerID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Worker().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	AddWorkerHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *Worker) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := AuthSlice{&local}
	if err = local.L.LoadWorker(ctx, tx, false, (*[]*Auth)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Worker == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Worker = nil
	if err = local.L.LoadWorker(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Worker == nil {
		t.Error("struct should have been eager loaded")
...
func testAuthToOneSetOpWorkerUsingWorker(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, workerDBTypes, false, strmangle.SetComplement(workerPrimaryKeyColumns, workerColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, workerDBTypes, false, strmangle.SetComplement(workerPrimaryKeyColumns, workerColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Worker{&b, &c} {
		err = a.SetWorker(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if exists, err := AuthExists(ctx, tx, a.WorkerID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	authDBTypes = map[string]string{`WorkerID`: `varchar`, `Password`: `varchar`, `Session`: `varchar`, `ChangePasswordCount`: `int`, `FailAuthCount`: `int`, `Status`: `int`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_           = bytes.MinRead
)
